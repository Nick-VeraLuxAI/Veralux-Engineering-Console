...
function threshold(status: AcceptanceThresholdVerdict["status"]): AcceptanceThresholdVerdict {
  return {
    ...
    status,
    ready: status === "ready_for_user_approval" || status === "passed_with_skips",
    approval_allowed: status === "ready_for_user_approval" || status === "passed_with_skips",
    risk_level: "low",
    risk_notes: [],
    normalized_gates: [],
    threshold_input: {
      ...
      command_results: [],
      skipped_checks: [],
      diff_scope_safety_status: "passed",
    ...
    passed_gates: status === "ready_for_user_approval" || status === "passed_with_skips" ? ["task_tests"] : [],
    failed_gates: status === "failed" ? ["task_tests"] : [],
    skipped_gates: [],
    blocked_gates: status === "blocked" ? ["role_policy"] : [],
    warnings: [],
    unresolved_issues: status === "ready_for_user_approval" ? [] : ["A gate needs attention."],
    blocking_reasons: status === "blocked" ? ["role_policy blocked"] : [],
    approval_required: true,
    integration_allowed: false,
    integration_performed: false,
...
function revisionLoop(status: PrototypeRevisionLoopResult["status"]): PrototypeRevisionLoopResult {
  return {
    ...
    rounds: [],
    final_readiness_verdict: status === "ready_for_user_approval" ? "ready_for_user_approval" : "failed",
