...
const PATCH_PROPOSAL_FORBIDDEN_AUDIT_EVENT_TYPES = new Set<string>([
...
export type VeraPatchProposalReadinessCheck = {
  id: string;
  ok: boolean;
  message: string;
};

export type VeraPatchProposalReadinessResult = {
  ok: boolean;
  safeToCreateProposal: boolean;
  reasons: string[];
  checks: VeraPatchProposalReadinessCheck[];
  runId: string;
  taskId: string;
  veraWorkOrderId: string | null;
  sourceArtifactPath: string | null;
  sourceArtifactHash: string | null;
  expectedSourceArtifactHash: string | null;
...
function resolveSourceArtifactPath(
  runId: string,
  governanceNotes: VeraRunGovernanceNotes,
): string | null {
...
function forbiddenReleaseAuditEvents(runId: string): string[] {
  return listAuditEventsForRun(runId)
    .filter((event) => PATCH_PROPOSAL_FORBIDDEN_AUDIT_EVENT_TYPES.has(event.eventType))
    .map((event) => event.eventType);
}

export function assessVeraPatchProposalReadiness(
  runId: string,
): VeraPatchProposalReadinessResult {
  const checks: VeraPatchProposalReadinessCheck[] = [];
  ...
  if (!run) {
    return {
      ...
      expectedSourceArtifactHash: null,
      run: null,
      task: null,
      governanceNotes: {},
    };
  ...
  addCheck(
    checks,
    reasons,
    "artifact_approved_step",
    run.currentStep === VERA_IMPLEMENTATION_ARTIFACT_APPROVED_STEP,
    `Run currentStep is ${VERA_IMPLEMENTATION_ARTIFACT_APPROVED_STEP}.`,
    `Run must be in ${VERA_IMPLEMENTATION_ARTIFACT_APPROVED_STEP} before patch proposal creation.`,
  );
  ...
  addCheck(
    ...
    "Vera implementation artifact must be approved before patch proposal creation.",
  );

  addCheck(
    checks,
    reasons,
    "no_existing_patch_proposal",
    !hasVeraImplementationPatchProposal(run.governanceNotes),
    "No prior Vera patch proposal exists.",
    "Vera patch proposal already exists for this run.",
  );

  const sourceArtifactPath = resolveSourceArtifactPath(run.id, governanceNotes);
  addCheck(
    checks,
    reasons,
    "source_artifact_path_known",
    Boolean(sourceArtifactPath?.trim()),
    "Implementation artifact path is known.",
    "Implementation artifact path is missing.",
  );

  let sourceArtifactHash: string | null = null;
  const sourceExists = Boolean(sourceArtifactPath && fs.existsSync(sourceArtifactPath));
  addCheck(
    checks,
    reasons,
  ...
  const expectedSourceArtifactHash =
    governanceNotes.veraImplementationArtifactHash?.trim() ?? null;
  if (sourceExists && sourceArtifactPath) {
    try {
      const content = fs.readFileSync(sourceArtifactPath, "utf8");
      sourceArtifactHash = hashArtifactContent(content);
      if (expectedSourceArtifactHash) {
        addCheck(
          checks,
          reasons,
          "source_artifact_hash_matches",
  ...
  return {
    ok: reasons.length === 0,
    safeToCreateProposal: reasons.length === 0,
    reasons,
    checks,
    runId: run.id,
    taskId: run.taskId,
