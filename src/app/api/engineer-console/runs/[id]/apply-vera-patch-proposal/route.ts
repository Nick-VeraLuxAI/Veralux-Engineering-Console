...
} from "@/lib/engineer-console/bridge/apply-vera-implementation-patch-proposal";
...
export async function POST(
  ...
  try {
    ...
    if (error instanceof VeraImplementationPatchApplicationError) {
      return NextResponse.json(
        {
          error: error.message,
          code: error.code,
          safeToApplyPatch: error.safeToApplyPatch,
          reasonCodes: error.reasonCodes,
          warning: "No patch was applied. Repository files were not mutated.",
        },
        { status: error.status },
      );
    }
    console.error("Vera patch application error:", error);
    return NextResponse.json(
      { error: "Failed to apply Vera patch proposal." },
      { status: 500 },
    );
  }
}
