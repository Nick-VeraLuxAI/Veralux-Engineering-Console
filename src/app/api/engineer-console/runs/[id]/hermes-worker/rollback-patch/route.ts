...
} from "@/lib/engineer-console/hermes-worker/apply-hermes-patch";
...
export async function POST(
  ...
  try {
    const result = rollbackHermesPatchForRun({
      runId,
      dispatchId,
      operatorApproval: {
    ...
    console.error("Hermes rollback patch error:", error);
    return NextResponse.json({ error: "Failed to rollback Hermes patch" }, { status: 500 });
  }
}
