...
import { createLocalCommitForRun } from "@/lib/engineer-console/governance/commit-candidate/create-local-commit";
import { LocalCommitError } from "@/lib/engineer-console/governance/commit-candidate/validate-local-commit-for-run";
...
export async function POST(
  ...
  try {
    const result = await createLocalCommitForRun({
      ...
      commitMessageOverride: body.commitMessageOverride,
    });
    return NextResponse.json(result);
  } catch (error) {
    if (error instanceof LocalCommitError) {
    ...
    return NextResponse.json({ error: "Failed to create local commit" }, { status: 500 });
  }
}
