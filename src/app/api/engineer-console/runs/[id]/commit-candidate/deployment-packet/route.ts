...
import { prepareDeploymentPacketForRun } from "@/lib/engineer-console/governance/commit-candidate/prepare-deployment-packet";
import { DeploymentPacketError } from "@/lib/engineer-console/governance/commit-candidate/validate-deployment-packet-for-run";
...
export async function POST(
  ...
  try {
    ...
    if (error instanceof DeploymentPacketError) {
      return NextResponse.json(
        { error: error.message, code: error.code },
        { status: error.status },
      );
    }
    console.error("Deployment packet preparation error:", error);
    return NextResponse.json({ error: "Failed to prepare deployment packet" }, { status: 500 });
  }
}
