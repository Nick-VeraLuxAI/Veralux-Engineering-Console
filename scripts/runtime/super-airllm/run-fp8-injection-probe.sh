...
LOG_FILE="${LOG_DIR}/super-fp8-injection-probe.log"

if [[ ! -x "${PYTHON}" ]]; then
  echo "Missing ${PYTHON}; AirLLM FP8 injection probe requires .venv-airllm" >&2
  exit 1
fi
...
CMD=(timeout "${PROBE_TIMEOUT_SECONDS}" "${PYTHON}" -m airllm.fp8_injection_probe_cli "$@")

if [[ "${FP8_INJECTION_PROBE_FOREGROUND:-}" == "1" ]]; then
  exec "${CMD[@]}"
fi
...
if [[ ${exit_code} -eq 124 ]]; then
  echo '{"phase":"super_airllm_repair_s6_fp8_injection_probe","verdict":"fp8_injection_probe_timeout","failure_classification":"timeout"}' >&2
fi
